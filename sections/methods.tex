...
We leveraged the secondary structure of these sequences to achieve this.
...
We can also transform each RNA sequence into a multivariate time series using grouped binary encoding, where nucleotides are grouped into (A, U) and (G, C). 
Each time series channel indicates the presence of a nucleotide from the respective group at each time step.
...
To conclude, the single value mapping and its two variations: grouped variable-length channel mapping and grouped fixed-length channel mapping, allow us to focus on the absolute values of the alphabet and ignore what has happened in the past.
