...
While the 1D sequence of these molecules can be easily obtained by sequencing, it is hard to measure the 3D structure (tertiary structure) from the experiment and predict it directly from the 1D sequence.
Many relevant properties of the 3D structure are typically investigated by analyzing the 1D sequence and the predicted 2D structure derived from it, known as secondary structure. 
...
In response to this, we proposed a multivariate time series classification-based method to provide a simpler, more computationally efficient framework for this task.
% Contributions
...
\begin{enumerate}
    ...
    \item We introduced nine encoding methods to convert RNA sequences to time series. 
