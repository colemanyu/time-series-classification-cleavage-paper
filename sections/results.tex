...
We applied padding to the shorter channels using the constant value 100, which does not appear in the time series before padding. It ensures the padding does not introduce ambiguity or interfere with the semantic meaning of the encoded nucleotide signals.
...
The RNA strand and its complementary strand are each encoded into one or two channels, using the methods in Tables~\ref{tab:ts_encoding}, \ref{tab:ts_encoding_prob}.
For example, single value mapping encodes each of them in one channel, while grouped variable-length channel mapping encodes in two channels.
...
\end{table}
We used the single value mapping as the encoding method.
...
In our downstream analysis, we adopted the combination of RNA sequence time series, secondary structure time series, and base-pair probability time series as our multivariate time series input.
...
\end{figure}
To summarize Table~\ref{tab:results}, we plot the CD diagrams for the best classifier, as shown in Figure~\ref{fig:best_model_cd}, and the best encoding method, as shown in Figure~\ref{fig:best_encoding_cd}.
